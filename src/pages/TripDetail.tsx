import { AIChatPanel, AIItineraryDialog } from '@/components/ai'
...
import { useCurrentPlans, useCurrentTrip, useTripLoading, useTripStore } from '@/stores/tripStore'
import { toast } from '@/stores/uiStore'
import type { GeneratedItinerary } from '@/types'
import { PLAN_TYPE_ICONS } from '@/utils/constants'
import { formatDateRange, formatTime, getTripDuration } from '@/utils/format'
import { downloadItineraryPDF } from '@/utils/pdfExport'
import { downloadGPX, downloadKML } from '@/utils/routeExport'
import { getTimezoneFromCountry, getTripDayDate, getTripStatus } from '@/utils/timezone'
import {
  ArrowLeft,
  BookOpen,
  Calendar,
...
export function TripDetail() {
  ...
  const plansByDay = useMemo(() => {
    ...
    for (const day of Object.keys(grouped)) {
      grouped[Number.parseInt(day)].sort((a, b) => a.startTime.localeCompare(b.startTime))
    }
    return grouped
  }, [plans])
  ...
  // 진행 중 여행이면 오늘이 몇 일차인지 (아니면 null)
  const todayDayN = useMemo(() => {
    if (!trip) return null
    const status = getTripStatus(trip.startDate, trip.endDate)
    return status.kind === 'ongoing' ? Math.min(status.dayN, days.length || status.dayN) : null
  }, [trip, days])

  // 최초 로드 시 오늘(진행 중) 또는 Day 1 자동 펼침 — 여행당 1회만
  const autoExpandedTripRef = useRef<number | null>(null)
  useEffect(() => {
    if (!trip?.id || days.length === 0) return
    if (autoExpandedTripRef.current === trip.id) return
    autoExpandedTripRef.current = trip.id
    const target = todayDayN ?? 1
    setExpandedDays(new Set([target]))
    setActiveDay(target)
    if (target > 1) {
      // 오늘 섹션이 화면에 들어오도록 즉시 스크롤 (첫 페인트 직후)
      requestAnimationFrame(() => {
        dayRefs.current[target]?.scrollIntoView({ block: 'start' })
      })
    }
  }, [trip, days, todayDayN])
  ...
  useEffect(() => {
    ...
    days.forEach((day) => {
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) setActiveDay(day)
        },
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg sm:text-2xl font-bold text-[var(--foreground)] break-words">
                {trip.title}
              </h1>
          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-wrap gap-2 sm:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{getTripDuration(trip.startDate, trip.endDate)}일</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            >
              {todayDayN && (
                <button
                  type="button"
                  onClick={() => {
                    setActiveDay(todayDayN)
                    setExpandedDays((prev) => new Set(prev).add(todayDayN))
                    dayRefs.current[todayDayN]?.scrollIntoView({
                      behavior: 'smooth',
                      block: 'start',
                    })
                  }}
                  className="flex-shrink-0 self-stretch flex items-center gap-1 px-2.5 rounded-xl text-xs font-bold bg-primary-50 text-primary-600 ring-1 ring-inset ring-primary-500/30 dark:bg-primary-950/40 dark:text-primary-400 hover:bg-primary-100 dark:hover:bg-primary-950/60 transition-colors"
                  aria-label={`오늘 일정(Day ${todayDayN})으로 이동`}
                >
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 min-w-[2.75rem] flex flex-col items-center py-1.5 px-1 rounded-xl text-center transition-colors ${
                      activeDay === day
                        ...
                        : isToday
                          ? 'bg-zinc-100 dark:bg-zinc-800 text-primary-600 dark:text-primary-400 ring-1 ring-inset ring-primary-500/40 hover:bg-zinc-200 dark:hover:bg-zinc-700'
        ...
        <Dialog open={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)}>
          ...
          <DialogBody>
            <p className="text-zinc-600 dark:text-zinc-400">
              "{trip.title}" 여행을 삭제하시겠습니까? 이 작업은 되돌릴 수 없으며, 모든 일정도 함께
              삭제됩니다.
            </p>
          </DialogBody>
          <DialogActions>
